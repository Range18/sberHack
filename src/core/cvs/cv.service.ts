...
@Injectable()
export class CvService extends BaseEntityService<CVs, 'NotFoundExceptions'> {
  constructor(
    @InjectRepository(CVs)
    private readonly repository: Repository<CVs>,
  ) {
    super(
      repository,
  ...
  async getFileStream(userId: number) {
